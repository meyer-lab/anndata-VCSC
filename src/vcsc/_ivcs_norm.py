...
load_and_normalize` builds -- without ever materializing it. Centering
makes every implicit structural zero a nonzero value, so a real
materialization is an ``n_rows * n_cols`` dense array; the whole point of a
"view" here is to avoid paying for that until (and unless) the caller
...
- IVCSC (columns are the major/deduplicated axis): both passes collapse
  into one, fully parallel over columns with no cross-thread writes --
  each column's own elements carry everything needed to compute both its
  ``gene_scale`` and its ``col_mean`` (:func:`_column_stats_major_is_col`).
- IVCSR (rows are major, columns are minor): each pass is a scatter-add
  across columns from many rows, so it's parallelized like
  :mod:`vcsc._rapid_load`'s ``_scaled_col_sums`` -- row-chunked with
  thread-local partial column arrays, reduced by summing across threads
  (:func:`_scaled_col_sums_vcs`, :func:`_transformed_col_sums_vcs`).
"""
...
if TYPE_CHECKING:
    from vcsc._ivcs import _IVCSBase

__all__ = ["IVCSCArrayNormalized", "IVCSRArrayNormalized"]


# -- statistics: IVCSC (columns major) -- fused, no cross-thread writes -----
...
# -- statistics: IVCSR (rows major, columns minor) -- scatter-add passes ---
...
# which owns disjoint rows (IVCSR) or columns (IVCSC) of ``out``, so there's
# no cross-thread write.
...
class _IVCSNormalizedBase:
    ...
    def __init__(self, arr: _IVCSBase) -> None:
    ...
    def __getitem__(self, key: Any) -> np.ndarray:
        ...
        byte-packed for a major-axis-only slice); the transform/centering
        ...
        if isinstance(key, tuple):
            if len(key) != 2:
                raise IndexError("IVCSC/IVCSR normalized arrays are 2-D")
            row_key, col_key = key
        else:
